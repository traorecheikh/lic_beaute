...
export async function uploadProMedia(token: string, file: File) {
  return withApiError(async () => {
    ...
    if (!response.ok) {
      let code = "unknown_error";
      let message = "Téléversement impossible.";
      try {
    ...
    return (await response.json()) as { id: string; publicUrl: string; objectKey: string };
  });
}
