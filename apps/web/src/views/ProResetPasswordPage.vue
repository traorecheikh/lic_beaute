<template>
  <div class="min-h-screen bg-neutral-bg flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <h2 class="mt-6 text-center page-title">Réinitialisation du mot de passe</h2>
      <p class="mt-2 text-center row-meta">
        Définissez votre nouveau mot de passe pour accéder à votre espace professionnel.
      </p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div class="panel-clean py-8 px-4 sm:px-10">
        ...
        <div v-if="linkInvalid" class="text-center space-y-4">
          <p class="row-primary text-error">Ce lien de réinitialisation est invalide ou a expiré.</p>
...
async function handleReset() {
  ...
  loading.value = true;
  try {
    await proAuth.loginWithResetToken(token.value, email.value, password.value);
    toast.success("Mot de passe réinitialisé ! Bienvenue sur Beauté Avenue.");
